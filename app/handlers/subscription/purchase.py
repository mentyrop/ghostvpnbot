    ...
    try:
        ...
        if trial_tariff and trial_tariff.allowed_squads:
            ...
            if trial_squads:
                ...
                # Нет trial_eligible сквадов для отображения в оффере; при активации будет использован дефолтный сквад
                logger.debug(
                    'Нет сквадов для отображения в оффере триала (при активации используется дефолтный сквад при необходимости)',
                    user_id=db_user.id,
                )
