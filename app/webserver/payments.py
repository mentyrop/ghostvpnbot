...
def create_payment_router(bot: Bot, payment_service: PaymentService) -> APIRouter | None:
    ...
    # Robokassa Result URL (уведомление об оплате; Robokassa присылает GET с OutSum, InvId, SignatureValue)
    if settings.is_robokassa_enabled():
        ...
        async def robokassa_webhook(request: Request) -> Response:
            ...
            # Если нет параметров платежа — отдаём health (для проверки URL)
